...
```java
// 核心注解中的 AutoConfigurationPackage 注解
@Import(AutoConfigurationPackages.Registrar.class)  // 使用 @Import 注解给 ioc 中导入一个 AutoConfigurationPackages.Registrar 组件
...
```java
// @AutoConfigurationPackage 注解中使用 @Import 导入的类	
...
/**
	...
	static class Registrar implements ImportBeanDefinitionRegistrar, DeterminableImports {
      ...
      // 这里的 metadata 指代的是 当前spring boot项目的启动主类
      // new PackageImport(metadata).getPackageName() 可以获取到 主类的包名
      // register() 负责把主类包名下的所有组件一起注册到ioc容器中
...
```java
    ...
    // getAutoConfigurationEntry() 给容器中批量导入一些组件，获取所有需要导入到ioc中的配置类全限定路径
...
- `List<String> configurations = getCandidateConfigurations(annotationMetadata,attributes);`获取候选的配置
  ...
  - ```java
    ...
    // 特有 spring-boot-autoconfigure-2.2.6.RELEASE.jar 包下的 META-INF/spring.factories
    ```
  ...
  * 虽然127个场景的自动配置类在启动的时候都会全部加载
  * 根据条件装配原则，最终会按需配置`@Conditional`
...
public class DispatcherServletAutoConfiguration {
    ...
    // 同时这里还顺便配置了文件上传解析器
    // 给 @Bean 标注的方法传入参数，这个参数的值就会从 ioc 中寻找，找到之后再给 resolver 赋值
    // 然后再将赋值后的 resolver 放入到 ioc 容器中
    // 这么做的原因是：ioc容器汇总的 MultipartResolver 的名字必须是 multipartResolver
    // 但是有的用户往ioc中放入的 MultipartResolver 对象可能是任意名字，此时就不符合规范，不能正常使用
    // 而经过这里的处理，可以将其重命名为 multipartResolver，使其能正常使用
    @Bean
		@ConditionalOnBean(MultipartResolver.class) 																				// 当容器中有这个类型的组件
		@ConditionalOnMissingBean(name = DispatcherServlet.MULTIPART_RESOLVER_BEAN_NAME)    // 但是没有名字叫 multipartResolver 的组件
...
​	**5)、spring boot 默认会在底层配置好所有的组件，如果用户配置了自己的组件，就以用户配置的优先**
...
请求进来，先去找`controller`看能不能处理，不能处理的请求又都交给静态资源处理器进行处理，

如果静态资源都找不到，那么将返回`404`
